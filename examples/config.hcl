...
# Use env() function for environment variable interpolation in paths and values.
...
defaults {
  ...
  version = 2         # Default KV version (1 or 2, or omit for auto-detect)
...
# Note: Path supports ${env("VAR")} interpolation
secret "dev-database" {
...
# Production secrets with explicit version
secret "prod-database" {
  mount   = "secret"
  path    = "prod/database"
  version = 2  # Explicitly use KV v2
